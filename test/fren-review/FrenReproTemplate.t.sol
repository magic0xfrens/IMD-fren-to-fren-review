...
/// @title Fren Review Repro test template
///
...
///
/// Harness surface (test/probes/YBase.sol): `registry`, `hook`, `perp`, `pm`,
/// `token`, `untrusted caller`, `affected user`, `trader`; `_buy(ethIn, to)` pays from THIS
...
contract FrenReproTemplate is FrenBase {
    ...
    function test_FrenTemplate_StackBootsAndTrades() public {
        ...
        uint256 before = IERC20(token).balanceOf(outsider);
        uint256 got = _buy(1 ether, outsider);

        // Assert the EFFECT, never just that calls returned.
        assertGt(got, 0, "a real swap went through the hook");
        assertEq(IERC20(token).balanceOf(outsider) - before, got, "outsider received exactly what the swap reported");
    }
}
